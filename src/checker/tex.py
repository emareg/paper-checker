...
# LaTeX Rules
# --------------------------------------------
R_Caption_Period = ReRule(" * INFO: Captions should be ended by a period '.'.", '.}' ,r'\\caption\{[^}]+[^. ]\s*(\}\s*)(?=\n)')
...
def checkTeXreferences( text ):
    ...
    print(" * INFO: found {} references and {} citations.".format( len(refs.keys()), len(matches) ))
